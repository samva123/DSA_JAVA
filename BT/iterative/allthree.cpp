...
// Function to get Preorder, Inorder, and Postorder traversal in one go
vector<vector<int>> preInPostTraversal(Node* root) {
    // Result vector containing pre, in, and post-order traversals
    vector<vector<int>> ans(3); // ans[0] -> pre, ans[1] -> in, ans[2] -> post

    if (root == NULL) {
        return ans; // Return an empty traversal structure
    }
    ...
    while (!st.empty()) {
        ...
        if (it.second == 1) {
            ans[0].push_back(it.first->data); // Preorder
            it.second = 2;
            st.push(it);
            if (it.first->left) st.push({it.first->left, 1});
        } 
        else if (it.second == 2) {
            ans[1].push_back(it.first->data); // Inorder
            it.second = 3;
            st.push(it);
            if (it.first->right) st.push({it.first->right, 1});
        } 
        else {
            ans[2].push_back(it.first->data); // Postorder
        }
    }
